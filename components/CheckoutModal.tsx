...
export default function CheckoutModal({ onClose }: { onClose: () => void }) {
  ...
  // === NOVA FUNÇÃO: INJEÇÃO DIRETA NO CRM ===
  const salvarNoCRM = async (nome: string, telefone: string, endereco: string) => {
    try {
      ...
      if (existente) {
        // Atualiza endereço e data do último pedido (ideal para quando troca de endereço)
        ...
        if (atualizado) {
          ...
          localStorage.setItem("@saas_customer", JSON.stringify(atualizado));
        }
      } else {
        // Cria um cliente totalmente novo
      ...
      console.error("Erro interno do CRM ao salvar cliente:", e);
    }
  };
  // ==========================================
  ...
  const handleFinalizarPedido = async () => {
    ...
    // Captura Nome
    let nomeClienteFinal = cliente?.name;
    if (!nomeClienteFinal || nomeClienteFinal === "Cliente Visitante") {
      nomeClienteFinal = prompt("Como podemos te chamar?");
    }
    if (!nomeClienteFinal) return;

    // Captura Telefone (Fundamental para o CRM e WhatsApp)
    let telefoneClienteFinal = cliente?.phone;
    if (!telefoneClienteFinal || telefoneClienteFinal === "Não informado") {
      telefoneClienteFinal = prompt("Qual o seu WhatsApp? (Apenas números com DDD)");
    }
    if (!telefoneClienteFinal) return;
    ...
    try {
      ...
      // 1. MÁGICA DO CRM: Salva ou atualiza o cliente antes de gerar o pedido
      await salvarNoCRM(nomeClienteFinal, telefoneClienteFinal, enderecoFinal);

      // 2. Continua gerando o pedido exatamente como você configurou
      const { data: pedidoSalvo, error } = await supabase.from("orders").insert([{ tenant_id: tenantId, customer_name: nomeClienteFinal, customer_address: enderecoFinal, payment_method: metodoFinal, total_amount: totalGeral, status: 'pendente' }]).select().single();
