...
export default function DashboardInicio({ tenant }: { tenant: any }) {
  ...
  // SOLUÇÃO DO ERRO DE HYDRATION: Garante que a tela só renderiza após montar no navegador
  const [isMounted, setIsMounted] = useState(false);

  // Estados de Gamificação e Metas (Inicia com padrão e depois lê o cache)
  ...
  useEffect(() => {
    ...
    // Agora sim, busca o cache com segurança após o carregamento
    ...
    try {
      ...
      if (ordersData) {
        ...
        ordersData.forEach(pedido => {
          // TRAVA DE SEGURANÇA: Evita erro se pedido.order_items vier nulo da tabela
  ...
  // ==========================================
  const porcentagemMeta = metaMensal > 0 ? Math.min((faturamentoTotal / metaMensal) * 100, 100) : 0;
  
  let nivelAtual = "Iniciante";
  let corNivel = "text-zinc-500 bg-zinc-100";
  
  if (porcentagemMeta >= 100) { nivelAtual = "Diamante 💎"; corNivel = "text-cyan-700 bg-cyan-100"; }
  else if (porcentagemMeta >= 75) { nivelAtual = "Ouro 🥇"; corNivel = "text-amber-700 bg-amber-100"; }
  else if (porcentagemMeta >= 50) { nivelAtual = "Prata 🥈"; corNivel = "text-slate-700 bg-slate-200"; }
  else if (porcentagemMeta >= 25) { nivelAtual = "Bronze 🥉"; corNivel = "text-orange-800 bg-orange-100"; }

  useEffect(() => {
    if (!loading && isMounted) setTimeout(() => setProgressoAnimado(porcentagemMeta), 300);
  }, [loading, porcentagemMeta, isMounted]);
