...
import { UploadCloud, Loader2, Image as ImageIcon, CheckCircle2, AlertCircle, Clock } from "lucide-react";
...
export default function ConfigLoja({ tenant, onUpdate }: { tenant: any, onUpdate: () => void }) {
  ...
  // Estados: Horários (business_hours)
  const defaultHours = {
    ...
    try {
      ...
      const { error } = await supabase
        ...
        .update({ 
          ...
          business_hours: horarios // SALVANDO OS HORÁRIOS NO BANCO
        })
        .eq("id", tenant.id);
  ...
  return (
    <div className="bg-white rounded-xl shadow-sm border border-zinc-200 overflow-hidden min-h-[600px] animate-in fade-in">
      <div className="border-b border-zinc-200 px-6 pt-4 flex gap-6 bg-zinc-50/50 overflow-x-auto">
        <button onClick={() => setAbaAtiva("sobre")} className={`font-bold text-sm pb-4 -mb-px border-b-2 transition-colors whitespace-nowrap ${abaAtiva === "sobre" ? "border-indigo-600 text-indigo-700" : "border-transparent text-zinc-500 hover:text-zinc-800"}`}>
           Sobre a Loja & Visual
        </button>
        <button onClick={() => setAbaAtiva("horarios")} className={`font-bold text-sm pb-4 -mb-px border-b-2 transition-colors whitespace-nowrap ${abaAtiva === "horarios" ? "border-indigo-600 text-indigo-700" : "border-transparent text-zinc-500 hover:text-zinc-800"}`}>
           Horários de Funcionamento
        </button>
        <button onClick={() => setAbaAtiva("logistica")} className={`font-bold text-sm pb-4 -mb-px border-b-2 transition-colors whitespace-nowrap ${abaAtiva === "logistica" ? "border-indigo-600 text-indigo-700" : "border-transparent text-zinc-500 hover:text-zinc-800"}`}>
           Área de Entrega
        </button>
      </div>
      ...
      <div className="p-8">
            ...
            <div>
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <input type="text" value={nome} onChange={e => setNome(e.target.value)} className="w-full border border-zinc-300 rounded-lg p-3 text-sm focus:ring-2 focus:ring-indigo-500 outline-none" />
                </div>
                <div>
                  <label className="block text-sm font-bold text-zinc-700 mb-1">WhatsApp de Atendimento (Com DDD)</label>
                  <input type="text" value={whatsapp} onChange={e => setWhatsapp(e.target.value)} placeholder="Ex: 11999999999" className="w-full border border-zinc-300 rounded-lg p-3 text-sm focus:ring-2 focus:ring-indigo-500 outline-none" />
                </div>
                <div>
                  <label className="block text-sm font-bold text-zinc-700 mb-1">Endereço Físico</label>
                  <textarea value={endereco} onChange={e => setEndereco(e.target.value)} rows={2} className="w-full border border-zinc-300 rounded-lg p-3 text-sm focus:ring-2 focus:ring-indigo-500 outline-none resize-none" />
                </div>
                <button onClick={salvarDadosLoja} disabled={loading} className="bg-indigo-600 hover:bg-indigo-700 text-white font-bold py-3 px-8 rounded-lg transition-colors mt-2 flex items-center gap-2 shadow-sm">
        ...
        {abaAtiva === "horarios" && (
          <div className="max-w-3xl space-y-6 animate-in fade-in">
            <div className="bg-indigo-50 border border-indigo-100 p-5 rounded-xl flex items-start gap-4">
               <Clock className="text-indigo-600 shrink-0 mt-0.5" />
               <div>
                 <h3 className="font-bold text-indigo-900">Gestão de Horários</h3>
                 <p className="text-sm text-indigo-800 mt-1 leading-relaxed">
                   Defina os dias e horários em que sua loja está operando. O cardápio exibirá "Aberto" ou "Fechado" automaticamente e bloqueará pedidos fora do expediente.
                 </p>
               </div>
            </div>

            <div className="bg-white border border-zinc-200 rounded-xl shadow-sm overflow-hidden">
              <div className="grid grid-cols-12 gap-4 p-4 border-b border-zinc-100 bg-zinc-50/50 text-xs font-bold text-zinc-500 uppercase tracking-wider hidden sm:grid">
                ...
                {diasSemana.map((dia) => {
                  ...
                  return (
                    <div key={dia.key} className={`grid grid-cols-1 sm:grid-cols-12 gap-4 p-4 items-center transition-colors ${!data.ativo ? "bg-zinc-50/80" : ""}`}>
                      <div className="col-span-4 font-bold text-zinc-800">
                        {dia.label}
                      </div>
                      ...
                      <div className="col-span-3">
                        <input 
                          ...
                          disabled={!data.ativo}
                          className="w-full border border-zinc-300 rounded-lg p-2.5 text-sm outline-none focus:ring-2 focus:ring-indigo-600 disabled:opacity-50 disabled:bg-zinc-100" 
                        />
                      </div>
            ...
            <div className="pt-2">
              <button onClick={salvarDadosLoja} disabled={loading} className="w-full sm:w-auto bg-indigo-600 hover:bg-indigo-700 text-white font-bold py-3 px-8 rounded-lg transition-colors shadow-sm">
        ...
        {abaAtiva === "logistica" && (
          <div className="max-w-2xl space-y-6 animate-in fade-in">
            <div className="bg-indigo-50 border border-indigo-200 p-5 rounded-xl flex items-start gap-4">
               <span className="text-3xl">🛰️</span>
               <div>
                 <h3 className="font-bold text-indigo-900">Motor Geográfico Ativado</h3>
                 <p className="text-sm text-indigo-800 mt-1">Endereços fora da quilometragem máxima serão bloqueados no Checkout.</p>
               </div>
            </div>
            <div className="space-y-5">
              <div>
                <label className="block text-sm font-bold text-zinc-700 mb-1">CEP de Origem (Endereço da Loja) *</label>
                <input type="text" maxLength={8} value={cep} onChange={e => setCep(e.target.value.replace(/\D/g, ''))} placeholder="00000000" className="w-full sm:w-2/3 border border-zinc-300 rounded-lg p-3 text-sm focus:ring-2 focus:ring-indigo-500 outline-none" />
              </div>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm font-bold text-zinc-700 mb-1">Raio Máximo Seguro (KM) *</label>
                  <div className="relative">
                    <input type="number" value={raioEntrega} onChange={e => setRaioEntrega(Number(e.target.value))} className="w-full border border-zinc-300 rounded-lg p-3 pr-12 text-sm focus:ring-2 focus:ring-indigo-500 outline-none" />
                ...
                <div>
                  ...
                  <div className="relative">
                    ...
                    <input type="text" value={taxaEntrega} onChange={e => setTaxaEntrega(e.target.value)} className="w-full border border-zinc-300 rounded-lg p-3 pl-10 text-sm focus:ring-2 focus:ring-indigo-500 outline-none" />
                  </div>
                </div>
              </div>
              <div className="pt-4">
